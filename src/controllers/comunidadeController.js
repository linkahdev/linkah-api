...
export const getComunidadesVitrine = async (req, res) => {
  try {
...
export const enviarMensagem = async (req, res) => {
  const { evento_id, usuario_nome, usuario_foto, texto, imagem, tipo, status } = req.body;
  ...
  try {
    ...
    const result = await db.query(`
      ...
      (evento_id, usuario_nome, usuario_foto, texto, imagem, tipo, status, is_host)
...
export const listarMensagensPorEvento = async (req, res) => {
  const { evento_id } = req.params;

  try {
    const result = await db.query(`
      SELECT
        id, evento_id, usuario_nome,
        COALESCE(usuario_foto, $2) AS usuario_foto,
        texto, imagem, tipo, status, is_host, criado_em
      FROM public.mensagens_v2
      WHERE evento_id = $1
  ...
  try {
    if (usuario_nome && usuario_nome !== 'undefined' && usuario_nome !== 'null') {
      await db.query(`
        ...
        (evento_id, usuario_nome, usuario_foto, status, ultima_vez)
        VALUES ($1,$2,$3,$4,NOW())
      ...
      `,
      [id, usuario_nome, foto || AVATAR_FALLBACK, status || '✨', AVATAR_FALLBACK]);
    }
...
export const getTodasComunidades = async (req, res) => {
  try {
